...
def main():
    ...
    for i in range(n_epochs+1):
        ...
        if i%savestep == 0:
            ...
            X_loc, X_test, Y_test = data.testbatch(500) # Testing on a batch size of 500
    ...
    if Flag == True:
        ...
        print('')
        print('Test Dataset')
        X_loc, X_test, X_test_ic, Y1000_test = data.integratedbatch()
        X_test = encode_decode(ae_model, X_test, 'test').numpy()
        X_test = np.concatenate((X_test,X_test_ic), axis=-1)  
        Y_pred = don_model(X_test, X_loc)
        Y_pred = np.reshape(Y_pred, (-1,args.latent_dim))
        Y_pred = ae_model.decode(Y_pred)
        Y_pred = data.denormalize_ae(Y_pred)
        Y_pred = np.reshape(Y_pred, (X_test.shape[0],4,num_species))
        error = np.sum(np.abs(Y1000_test - Y_pred[:,3:4,:])**2)/np.sum(np.abs(Y1000_test)**2)

        save_dict = {'ref': Y1000_test, 'pred': Y_pred[:,3:4,:], 'error': error}
        io.savemat(pred_dir + 'test_results.mat', save_dict)   

        print('L2 norm of relative error: ', error)
        np.savetxt(errordir + 'error_DON_seed_' + str(seed_value) + '.txt', 
                                    np.expand_dims(np.array(error), axis=0), fmt='%e') 
        print('--------Complete--------')
